...
async def fry(args):     """ For .fry command, fries stickers or creates new ones. """     if not args.text[0].isalpha() and args.text[0] not in ("/", "#", "@", "!"):         user = await bot.get_me()         if not user.username:             user.username = user.first_name         message = await args.get_reply_message()         photo = None          if message and message.media:             if isinstance(message.media, MessageMediaPhoto):                 photo = io.BytesIO()                 photo = await bot.download_media(message.photo, photo)                 emojibypass = False             elif "image" in message.media.document.mime_type.split('/'):                 photo = io.BytesIO()                 await bot.download_file(message.media.document, photo)         else:             await args.edit("`Reply to photo/sticker to fry it bruh`")             return          if photo:             image = await resize_photo(photo)             image = await deepfry(                 img=image,                 url_base='westcentralus',
#                token='a8fad04b5c2f468781a22e2b7faXXXXX'             )             image.save("Sticker.png")         await args.client.send_file(await args.client.get_input_entity(args.chat_id), "Sticker.p>         await args.delete()  
...
HELPER.update({     "fry": "Fry. Fry stickers, photos. Based on @rupansh's kang command, with help of @jeepeo"
})
